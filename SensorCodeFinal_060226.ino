...
// ************************ SENSOR SETTINGS *******************************
char groupID[] = "$PH";    // EC(0...9)  PH(10...19)  TH(20...29)  THCO2(30...39)  SH(40...49)  CV(50...59)   TEST
char sensID[] = "10";       // 0  1  2  3 .... 59
long sendPeriodMinutes = 2;
...
void loop() {
   Serial.println("Wake!");
  if (millis() - tmr5 > sendPeriodMinutes * 60 * 1000 * 3) {  // ja atbildes no RPi nav ilgāk par 3 sūtīšanas periodiem/mēģinājumiem, taisam restartu
  ...
  if (strcmp(groupID, "$SH") == 0) {
    ...
    for (int i = 0; i < CHAN_COUNT; i++) { // for each channel
      ...
      //************Kā Ričardam bija*****************
      // kā piedāvā chat GPT
      //      float frequency = capa[i] * 0.149f * 1e-6f; // Hz
      //      float omega = 2.0f * 3.14159265f * frequency; // ω = 2πf
      //      capacitance = 1.0f / (omega * omega * 0.000018f); // F
      //      capacitance *= 1e9f; // μF

      // paņemot pa tiešo ričarda kodu tikai iazstājot 10^12 (priekš pf) uz 1000000 (10^6) priekš uF
      float frequency = capa[i] * 0.149f;
      frequency = frequency / 1000000.0f;
      capacitance = 2.0f * 3.14159265359f * frequency;
      capacitance = capacitance * capacitance;
      capacitance = 1000000.0f / (capacitance * 0.000018f);
      ...
      String outputstring = "";
      outputstring = outputstring + "Capacitance: " + String(capacitance) + " μF, Moisture: " + String(moisture) + "%";
    ...
    strcat(sendData, "/");
    ltoa(long(capacitance), pData, DEC);
    strcat(sendData, pData);
  ...
  if (answRec == true) {
    ...
    if (targetSecond * 1000 - (epochSeconds * 1000 + epochMilliseconds) < -30000) {
     ...
     // else (sleepCorrection = targetSecond * 1000 - (epochSeconds * 1000 + epochMilliseconds));
    Serial.print("Correction:");
    Serial.println(sleepCorrection);
    if (abs(sleepCorrection) > 30000) {
      sleepCorrection = 0;
    }
    Serial.print("Correction:");
    Serial.println(sleepCorrection);
